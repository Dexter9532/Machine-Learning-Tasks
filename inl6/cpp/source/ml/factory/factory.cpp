...
// -----------------------------------------------------------------------------
ConvLayerPtr Factory::convLayer(const std::size_t inputSize,
                                const std::size_t kernelSize,
                                const act_func::Type actFunc) {
  ///! @todo Replace conv_layer::ConvStub with conv_layer::Conv when
  /// implemented.
  return std::make_unique<conv_layer::Conv>(inputSize, kernelSize, actFunc);
}

// -----------------------------------------------------------------------------
DenseLayerPtr Factory::denseLayer(const std::size_t inputSize,
...
FlattenLayerPtr Factory::flattenLayer(const std::size_t inputSize) {
  ///! @todo Replace flatten_layer::Stub with flatten_layer::Flatten when
  /// implemented.
  return std::make_unique<flatten_layer::Stub>(inputSize);
}

// -----------------------------------------------------------------------------
ConvLayerPtr Factory::maxPoolLayer(const std::size_t inputSize,
                                   const std::size_t poolSize) {
  ///! @todo Replace conv_layer::MaxPoolStub with conv_layer::MaxPool when
  /// implemented.
